...
export class EagleViewDataSource implements DataSource<EagleViewAccountReport> {
    ...
    constructor(
        ...
        this.pageInfo$ = responses.pipe(
            map((r) =>
                r
                    ? { totalCount: r.TotalOfReports, currentPage: 0, pageSize: 10 }
        ...
        this.searches$
            .pipe(
                ...
                switchMap((s) => {
                    ...
                    if (
                        searchTerm !== '' &&
                        (search.fieldName === 'DatePlaced')
                    ) {
                        searchTerm = this.formatDateForFilter(searchTerm);
                    }

                    const req: EagleViewAccountReportRequest = {
                        productsToFiterBy: search.fieldName === 'ReportProducts' ? (searchTerm ? [parseInt(searchTerm)] : [31, 32, 44, 55]) : [31, 32, 44, 55],
                        statusesToFilterBy: search.fieldName === 'ReportStatus' ? [searchTerm] : [],
                        subStatusToFilterBy: '',
