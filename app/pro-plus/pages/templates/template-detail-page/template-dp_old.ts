...
import { ITemplateItem, TemplateViewModel } from './template-view-model';
...
export class TemplateDetailPageComponent implements OnInit, OnDestroy {
  ...
  // model: any = new BehaviorSubject<any>( null );
  ...
  constructor(
    ...
    private readonly cartService: ShoppingCartService, //*** // REVIEW API CALL
    private readonly userService: UserService, //*** // REVIEW API CALL
    private readonly quoteService: QuoteService, //*** // REVIEW API CALL
    private readonly productService: ProductsService, //*** // REVIEW API CALL
    private readonly userNotifier: UserNotifierService, //*** // REVIEW API CALL
  ...
  async ngOnInit() {
    ...
    /* STEPS: -- Fetch Template by templateId, and account (required) -- */
    // this.route.snapshot.paramMap; // get route params (account, templateId)
    // templates api call as Obs: from( this.templateService.getTemplateDetail( acccount, templateId ) );
    // this.userService.getSessionInfo()
    ...
    /*  this.route.params
         .pipe()
         .subscribe( ( temp: any ) => {
             from( this.templateService.getTemplateDetail( temp.templateId, temp.accountId ) )
                 .pipe(
                     tap( ( t: any ) => {
                         //this.model.next( t );
                         // this.dataSource = ( t.result.templateItems );
                         this.getLogs();
                         //  ERROR CHECKING                             
                     } )
                 ).subscribe();
  ...
  private async populateUserInfo() {
    ...
    if ( isLoggedIn ) {
      this.userInfo = await this.userService.ensureAllUserInfo();
    }

    if ( this.userInfo.currentUser ) {
      const currentUser = this.userInfo.currentUser;
      const phoneNumber = currentUser.mobilePhoneNumber
      ...
      if ( currentUser.contactAddress ) {
        this.address1 = currentUser.contactAddress.address1
          ? currentUser.contactAddress.address1
          : '';
        this.address2 = currentUser.contactAddress.address2
          ? currentUser.contactAddress.address2
          : '';
        this.city = currentUser.contactAddress.city
          ? currentUser.contactAddress.city
          : '';
        this.state = currentUser.contactAddress.state
