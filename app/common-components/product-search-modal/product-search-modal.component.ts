...
export class ProductSearchModalComponent implements OnInit, OnDestroy, AfterViewInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this.searchModalService.isLoadingSearch$.subscribe( loading => {
    ...
    this.searchModalService.emptySearchResults$.subscribe( isEmpty => {
      this.emptySearchResults$.next( isEmpty )
    } )

    this.searchModalService.searchResults$.subscribe( searchR => {
